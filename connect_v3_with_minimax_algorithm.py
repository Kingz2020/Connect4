...
def minimax(board, depth, maximizingPlayer):
    ...
    if maximizingPlayer:
        ...
        for col in valid_locations:
            ...
            temp_board_copy = board.copy()
            drop_piece(temp_board_copy, col, row, AI_PIECE)
            new_score = minimax(temp_board_copy, depth - 1, False)[1]
            if new_score > value:
                value = new_score
                column = col
        return column, value
        ...
        for col in valid_locations:
            row = get_next_open_row(board, col)
            temp_board_copy = board.copy()
            drop_piece(temp_board_copy, col, row, PLAYER_PIECE)
            new_score = minimax(temp_board_copy, depth - 1, True)[1]
            if new_score < value:
                value = new_score
                column = col
        return column, value
